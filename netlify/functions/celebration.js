...
exports.handler = async function(event) {
  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      // Pull a value by its JotForm field name (e.g. "q2_q2_dropdown0"), object-safe
      const getField = (key) => {
        let val = raw[key];
        if (val && typeof val === 'object') val = val.text || val.value || Object.values(val).join(' ');
        return (val != null && String(val).trim() !== '') ? String(val).trim() : undefined;
      };
      // JotForm sends generic field names (q2_q2_dropdown0 etc.), so match by
      // BOTH the known field position AND keywords — whichever finds a value first.
      const cabin    = getField('q2_q2_dropdown0') || findField(['cabin']);
      const occasion = getField('q3_q3_dropdown1') || findField(['occasion']);
      const guestName= getField('q4_q4_textbox2')  || findField(['guestname', 'guest', 'name']);
      const message  = getField('q5_q5_textarea3') || findField(['message', 'msg']);
      const showVal  = getField('q6_q6_dropdown4') || findField(['action', 'show']) || 'set';
      const action   = showVal.toLowerCase().includes('clear') || showVal.toLowerCase().includes('hide') ? 'clear' : 'set';
